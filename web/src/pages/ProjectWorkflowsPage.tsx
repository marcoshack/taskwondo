...
export function ProjectWorkflowsPage() {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      {/* Page Header */}
      <div>
        <h2 className="text-xl font-semibold text-[var(--foreground)]">{t('workflows.title')}</h2>
        <p className="mt-1 text-sm text-[var(--foreground-secondary)]">{t('workflows.description')}</p>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('workflows.definitionsTitle')}</h2>
            <p className="mt-1 text-sm text-[var(--foreground-secondary)]">{t('workflows.definitionsDescription')}</p>
          </div>
          {canManage && (
      ...
      {systemWorkflows.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-[var(--foreground-secondary)] mb-2 flex items-center gap-1.5">
          ...
          <div className="border border-[var(--border)] rounded-lg divide-y divide-[var(--border)]">
            {systemWorkflows.map((wf) => (
              <div key={wf.id} className="p-4">
                ...
                {wf.description && (
                  <p className="text-xs text-[var(--foreground-secondary)] mb-2">{wf.description}</p>
                )}
      ...
      {/* Project Workflows */}
      <div>
        <h3 className="text-sm font-medium text-[var(--foreground-secondary)] mb-2">
          {t('workflows.projectWorkflows')}
        </h3>
        {projectWorkflows.length === 0 ? (
          <div className="border border-dashed border-[var(--border)] rounded-lg p-6 text-center">
            <p className="text-sm text-[var(--foreground-secondary)]">{t('workflows.noProjectWorkflows')}</p>
            {canManage && (
      ...
      {/* Mapping */}
      <div>
        <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('workflows.mappingTitle')}</h2>
        <p className="mt-1 text-sm text-[var(--foreground-secondary)]">{t('workflows.mappingDescription')}</p>
      </div>

      {workflowError && <p className="text-sm text-[var(--danger)]">{workflowError}</p>}
      ...
      <div>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('escalation.title')}</h2>
            <p className="mt-1 text-sm text-[var(--foreground-secondary)]">{t('escalation.description')}</p>
          </div>
          {canManage && (
            <Button onClick={() => { setEditingEscalationId(null); setEscalationEditorOpen(true) }} className="border border-transparent">
              <span className="sm:hidden">{t('workitems.newShort')}</span>
              <span className="hidden sm:inline">{t('escalation.create')}</span>
            </Button>
          )}
        </div>
      </div>

      {escalationError && <p className="text-sm text-[var(--danger)]">{escalationError}</p>}

      {(!escalationLists || escalationLists.length === 0) ? (
        <div className="border border-dashed border-[var(--border)] rounded-lg p-6 text-center">
          <p className="text-sm text-[var(--foreground-secondary)]">{t('escalation.noLists')}</p>
        </div>
      ) : (
        <div className="border border-[var(--border)] rounded-lg divide-y divide-[var(--border)]">
            ...
            return (
              <div key={el.id} className="p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {isExpanded ? <ChevronDown className="h-4 w-4 text-[var(--foreground-muted)] shrink-0" /> : <ChevronRight className="h-4 w-4 text-[var(--foreground-muted)] shrink-0" />}
                    <span className="text-sm font-medium text-[var(--foreground)] truncate">{el.name}</span>
                  </button>
                ...
                {/* Row 2: level count badge + unmapped warning */}
                <div className="mt-1 ml-6 flex items-center gap-2">
                  <Badge color="gray">
                    {t('escalation.levelCount', { count: el.levels.length })}
                  </Badge>
                  {escalationMappings && !escalationMappings.some((m) => m.escalation_list_id === el.id) && (
                    <span className="inline-flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400">
                      <AlertTriangle className="h-3.5 w-3.5" />
                      {t('escalation.unmappedWarning')}
                    </span>
                  )}
                </div>
                      ...
                      .map((lv) => (
                        <div key={lv.id} className="flex items-center gap-3">
                          ...
                          {lv.users.length === 0 && (lv.teams ?? []).length === 0 ? (
                            <span className="text-xs text-[var(--foreground-muted)]">{t('escalation.noRecipients')}</span>
                          ) : (
      ...
      {/* Escalation Mapping */}
      <div>
        <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('escalation.mappingTitle')}</h2>
        <p className="mt-1 text-sm text-[var(--foreground-secondary)]">{t('escalation.mappingDescription')}</p>
      </div>

      {escalationMappingError && <p className="text-sm text-[var(--danger)]">{escalationMappingError}</p>}
      ...
      <div className="border border-[var(--border)] rounded-lg divide-y divide-[var(--border)]">
        {['task', 'ticket', 'bug', 'feedback', 'epic'].map((itemType) => {
          ...
          const hasSlaForType = slaTargets?.some((t) => t.work_item_type === itemType)
          return (
            <div key={itemType} className="p-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-[var(--foreground)]">
                  {t(`workitems.types.${itemType}`)}
                </span>
                <div className="flex items-center gap-2">
                  {hasEscalation && !hasSlaForType && (
                    <span className="hidden sm:inline-flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400">
                      <AlertTriangle className="h-3.5 w-3.5" />
                      {t('escalation.noSlaWarning')}
                    </span>
                  )}
                  {saved[`esc:${itemType}`] && (
                    <Check className="h-5 w-5 text-green-500 animate-[pulse_0.6s_ease-in-out_2]" />
                  ...
                  <Tooltip content={canManage ? t('sla.configure') : t('sla.view')}>
                    <button
                      className="rounded-md border border-[var(--border)] p-2 text-[var(--foreground-muted)] hover:text-[var(--primary)] dark:hover:text-[var(--primary)] disabled:opacity-50 disabled:cursor-not-allowed"
                      onClick={() => {
                        if (wfMapping?.workflow_id) {
                          setSlaModalType(itemType)
                          setSlaModalWorkflowId(wfMapping.workflow_id)
                        }
                      }}
                      disabled={!wfMapping?.workflow_id}
                    >
      ...
      {project && (
        <>
          <div>
            <h2 className="text-lg font-semibold text-[var(--foreground)]">{t('workflows.businessHoursTitle')}</h2>
            <p className="mt-1 text-sm text-[var(--foreground-secondary)]">{t('workflows.businessHoursDescription')}</p>
          </div>

          {bhError && <p className="text-sm text-[var(--danger)]">{bhError}</p>}
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 text-sm rounded-md border transition-colors disabled:opacity-60 disabled:cursor-not-allowed ${
                            currentDays.includes(d.value)
                              ? 'bg-[var(--primary-muted)] border-[var(--primary-border)] text-[var(--primary)]'
                              : 'bg-[var(--surface)] border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--surface-hover)]'
                          }`}
                          onClick={() => {
      ...
      {slaModalType && slaModalWorkflowId && (() => {
        ...
        return (
          <SLAConfigModal
            ...
            onSave={() => { showSaved(`esc:${slaModalType}`) }}
            projectKey={projectKey!}
...
function ScrollableRecipientList({
  ...
  return (
    <div className="flex items-center gap-1 min-w-0 flex-1">
      {canScrollLeft && (
        <button type="button" onClick={() => scroll('left')} className="shrink-0 p-0.5 text-[var(--foreground-muted)] hover:text-[var(--foreground-secondary)] dark:hover:text-[var(--foreground-muted)]">
      ...
      >
        ...
        {users.map((u) => (
          <span
            key={`user-${u.id}`}
            className="inline-flex items-center gap-1 shrink-0 rounded-full bg-[var(--primary-muted)] border border-[var(--primary-border)] dark:border-[var(--primary-border)] px-2 py-0.5 text-xs font-medium text-[var(--primary)]"
          >
      ...
      {canScrollRight && (
        <button type="button" onClick={() => scroll('right')} className="shrink-0 p-0.5 text-[var(--foreground-muted)] hover:text-[var(--foreground-secondary)] dark:hover:text-[var(--foreground-muted)]">
