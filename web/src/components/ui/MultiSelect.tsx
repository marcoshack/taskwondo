...
export function MultiSelect({ options, selected, onChange, placeholder = 'All', className = '' }: MultiSelectProps) {
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        type="button"
        className={`flex items-center justify-between w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
          selected.length > 0 && selected.length < options.length ? 'text-gray-900' : 'text-gray-500'
        }`}
        onClick={() => setOpen(!open)}
      >
        <span className="truncate">{label}</span>
        <svg className="ml-1 h-4 w-4 shrink-0 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {open && (
        <div className="absolute z-20 mt-1 w-full min-w-[180px] rounded-md border border-gray-200 bg-white shadow-lg">
            ...
            {hasGroups ? (
              groups.map((group) => (
                <div key={group.name ?? '__default'}>
                  {group.name && (
                    <div className="px-3 pt-2 pb-1 text-[0.65rem] font-semibold uppercase tracking-wider text-gray-400">
                      {group.name}
                    </div>
                  )}
                  {group.items.map((opt) => (
...
function OptionRow({ option, checked, onToggle }: { option: MultiSelectOption; checked: boolean; onToggle: (value: string) => void }) {
  return (
    <label className="flex items-center gap-2 px-3 py-1.5 hover:bg-gray-50 cursor-pointer text-sm text-gray-700">
      <input
        type="checkbox"
        checked={checked}
