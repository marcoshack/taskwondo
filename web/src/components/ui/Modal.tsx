...
export function Modal({ open, onClose, title, position = 'center', size = 'default', dismissable = true, className, containerClassName, children }: ModalProps) {
  ...
  return createPortal(
    <div className={`fixed inset-0 z-50 flex justify-center ${position === 'top' ? 'items-start pt-4' : 'items-center'} ${containerClassName ?? ''}`}>
      ...
      <div role="dialog" aria-modal="true" className={`relative bg-white dark:bg-gray-800/40 dark:backdrop-blur-sm rounded-lg shadow-xl ${size === 'full' ? 'w-[96vw] h-[96vh] p-0 flex flex-col overflow-hidden' : 'max-w-lg w-full mx-4 p-6 max-h-[90vh] overflow-y-auto overscroll-contain'} ${className ?? ''}`}>
        {title && (
