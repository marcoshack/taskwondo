...
export function WorkItemMobileCard({ item, statuses, showDates, assigneeName, inboxItemId, isWatching, onClick }: WorkItemMobileCardProps) {
  return (
    <div
    ...
    >
      ...
      {showDates && (
        <ScrollableRow className="mr-5 mt-1.5 text-xs text-gray-400 dark:text-gray-500" contentClassName="gap-3">
      ...
      {item.description && (
        <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400 truncate">{getDescriptionPreview(item.description)}</p>
      )}
    </div>
  )
}
