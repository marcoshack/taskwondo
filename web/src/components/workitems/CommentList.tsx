...
export function CommentList({ projectKey, itemNumber, sortOrder = 'desc', highlightedCommentId, onHighlightClear, onImageClick, onAttachmentLinkClick, draft, onDraftChange, readOnly = false, itemVisibility, onViewInline }: CommentListProps) {
  ...
  useEffect(() => { draftRef.current = newBody }, [newBody])
  ...
  return (
    <div className="space-y-4">
      {!readOnly && (
        <div ref={addCommentRef} className="space-y-2 pb-3 border-b border-[var(--border)]">
          <textarea
            ref={newCommentRef}
          ...
          <div className="flex items-center gap-3">
            ...
            {showVisibilityToggle && (
              <label className="flex items-center gap-1.5 text-xs text-[var(--foreground-secondary)] cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={commentVisibility === 'public'}
                  onChange={(e) => setCommentVisibility(e.target.checked ? 'public' : 'internal')}
                  className="h-3.5 w-3.5 rounded border-[var(--border)] text-[var(--primary)] focus:ring-[var(--focus-ring)] dark:border-[var(--border)] bg-[var(--surface)]"
                />
      ...
      {(sortOrder === 'desc' ? [...(comments ?? [])].reverse() : (comments ?? [])).map((c) => (
        <div
          ...
          className={`group/comment border-b border-[var(--border)] pb-3 rounded-md transition-colors duration-200 ${
            c.id === highlightedCommentId ? 'bg-[var(--primary-muted)] ring-1 ring-indigo-300 dark:ring-indigo-600 px-2 py-2 -mx-2' : ''
          }`}
        >
          {!addCommentVisible && (
            <div className="flex justify-end mb-1">
              <button
                className="text-xs text-[var(--foreground-muted)] hover:text-[var(--foreground-secondary)] dark:hover:text-[var(--foreground-muted)] flex items-center gap-1"
          ...
          {editingId === c.id ? (
            ...
            <>
              {c.anchor && (
                <div className="mb-2 ml-8 rounded-md border-l-4 border-[var(--primary-border)]  bg-[var(--primary-muted)]/40  px-3 py-2">
                  <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--foreground-secondary)] mb-1">
                    ...
                    <button
                      type="button"
                      className="ml-auto text-[var(--primary)] hover:underline text-xs"
                  ...
                  <pre className="text-xs text-[var(--foreground)] whitespace-pre-wrap break-words bg-[var(--surface)] border border-[var(--border)] rounded p-2 overflow-x-auto">
                    {c.anchor.snippet}
                  </pre>
                </div>
              )}
              <div className="flex items-center justify-between gap-2 mb-1">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-sm font-medium text-[var(--foreground)] shrink-0">{authorName(c.author_id)}</span>
                  <ScrollableDate date={c.created_at} />
                  {c.edit_count > 0 && <span className="text-xs text-[var(--foreground-muted)] italic shrink-0">{t('comments.editCount', { count: c.edit_count })}</span>}
                  {c.visibility !== 'internal' && (() => {
                    ...
                    return (
                      <span className={`inline-flex items-center gap-1 text-xs shrink-0 ${
                        ...
                        displayAs === 'public' ? 'text-[var(--danger)]' :
                        'text-[var(--primary)]'
                      }`}>
                ...
                <div className="flex items-center shrink-0">
                  {user && c.author_id === user.id && (
                    <button
                      className="group/edit relative inline-flex items-center justify-center w-7 h-7 rounded-md text-[var(--foreground-muted)] hover:text-[var(--foreground-secondary)] hover:bg-[var(--surface-tertiary)] hover:bg-[var(--surface-hover)] transition-colors sm:opacity-0 sm:group-hover/comment:opacity-100"
                  ...
                  {user && c.author_id === user.id && (
                    <button
                      className="group/del relative inline-flex items-center justify-center w-7 h-7 rounded-md text-red-400 hover:text-[var(--danger)] hover:bg-red-50 text-[var(--danger)] dark:hover:text-red-300 dark:hover:bg-red-900/30 transition-colors sm:opacity-0 sm:group-hover/comment:opacity-100"
              ...
              <div
                className={`prose prose-sm dark:prose-invert max-w-none text-[var(--foreground)] pl-8 rounded break-words ${user && c.author_id === user.id ? 'border border-transparent hover:border-[var(--border)] dark:hover:border-[var(--border)]' : ''}`}
                onDoubleClick={() => {
