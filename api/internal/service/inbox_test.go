...
func (m *mockInboxRepo) List(_ context.Context, userID uuid.UUID, _ bool, _ string, _ *uuid.UUID, limit int) (*model.InboxItemList, error) {
...
func TestInboxList_Success(t *testing.T) {
	...
	list, err := svc.List(context.Background(), info, false, "", nil, 50)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if list.Total != 3 {
...
func TestInboxList_IsolatedPerUser(t *testing.T) {
	...
	list1, _ := svc.List(context.Background(), user1, false, "", nil, 50)
	list2, _ := svc.List(context.Background(), user2, false, "", nil, 50)
