			...
			if err == nil {
				...
				if err := json.Unmarshal(setting.Value, &cfg); err == nil && cfg.ClientID != "" {
					...
					if err != nil {
						...
						case model.OAuthProviderOIDC:
							return NewOIDCProvider(cfg.ClientID, secret, redirectURI, cfg.IssuerURL, nil)
						}
					}
				}
			}
...
func (s *AuthService) SearchUsers(ctx context.Context, callerID uuid.UUID, query string) ([]model.UserWithNamespaces, error) {
	...
	users, err := s.users.Search(ctx, callerID, query)
	if err != nil {
...
func (s *AuthService) EnabledProviders(ctx context.Context) map[string]bool {
	...
	for _, name := range []string{model.OAuthProviderDiscord, model.OAuthProviderGoogle, model.OAuthProviderGitHub, model.OAuthProviderMicrosoft, model.OAuthProviderOIDC} {
		if s.isOAuthConfigured(ctx, name) {
			...
			case model.OAuthProviderOIDC:
				settingKey = model.SettingAuthOIDCEnabled
			}
			if settingKey != "" {
