...
func (r *ProjectRepository) GetByKey(ctx context.Context, key string) (*model.Project, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, name, key, description, item_counter, created_at, updated_at
...
func (r *ProjectRepository) ListByUser(ctx context.Context, userID uuid.UUID) ([]model.Project, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT p.id, p.name, p.key, p.description, p.item_counter, p.created_at, p.updated_at
		 FROM projects p
...
func (r *ProjectRepository) Update(ctx context.Context, project *model.Project) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE projects SET name = $1, key = $2, description = $3, updated_at = now()
		 WHERE id = $4 AND deleted_at IS NULL`,
		project.Name, project.Key, project.Description, project.ID)
	if err != nil {
...
func scanProject(row *sql.Row) (*model.Project, error) {
	...
	err := row.Scan(&p.ID, &p.Name, &p.Key, &description,
		&p.ItemCounter, &p.CreatedAt, &p.UpdatedAt)
...
func scanProjects(rows *sql.Rows) ([]model.Project, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Name, &p.Key, &description,
			&p.ItemCounter, &p.CreatedAt, &p.UpdatedAt); err != nil {
