...
func listWorkItemsTool() mcp.Tool {
	return mcp.NewTool("list_work_items",
		...
		mcp.WithArray("labels", mcp.WithStringItems(), mcp.Description("Filter by labels (AND semantics — item must have every label). Examples: agent:human-only, agent:awaiting-operator-input")),
