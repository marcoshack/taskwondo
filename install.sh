...
generate_env() {
    ...
    # Store generated values using _var_ prefixed variables (Bash 3 compatible)
    # Use _var_get / _var_set helpers instead of associative arrays
    _var_set() { eval "_var_$1=\"\$2\""; _var_all_keys="${_var_all_keys:-} $1"; }
    _var_get() { eval "echo \"\${_var_$1:-}\""; }
    _var_keys() { echo "${_var_all_keys:-}"; }
    _var_all_keys=""
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        case "$value" in
            ...
            __DERIVE:*__)
                ...
                for var_name in $(_var_keys); do
                    local_expr="${local_expr//\$\{$var_name\}/$(_var_get "$var_name")}"
                done
                value="$local_expr"
                ;;
            __OPTIONAL__)
...
if [[ -n "$IMPORT_FILE" ]]; then
    ...
    printf "  Admin email:     %s\n" "$(_var_get ADMIN_EMAIL)"
    printf "  Admin password:  %s\n" "$(_var_get ADMIN_PASSWORD)"
    echo
